...
(
	...
	(
		...
		(
			'mode'                    => 4,
			'fields'                  => array('round'),
			'headerFields'            => array('title', 'place', 'country'),
	...
	'palettes' => array
	(
		'default'                     => '{team_legend},germanTeam,germanCountry,opponentTeam,opponentCountry,home;{results_legend:hide},resultGerman,resultOpponent,info;{round_legend},round,pairing,eventDate,eventTime;{place_legend:hide},place;{referee_legend:hide},referee;{publish_legend},published'
	),
...
class tl_chesscompetition_matches extends Backend
{
	...
	{
		$temp = '<div class="tl_content_left"><b>'.$arrRow['round'].'</b> <i>'.$this->getDate($arrRow['eventDate']).'</i>';
		// Mannschaftsnamen bauen
		($arrRow['home']) ? $temp .= ' ' . $arrRow['germanTeam'] : $temp .= ' ' . $arrRow['opponentTeam']; 
		($arrRow['home']) ? $temp .= ' - ' . $arrRow['opponentTeam'] : $temp .= ' - ' . $arrRow['germanTeam']; 
		// Ergebnis bauen		
